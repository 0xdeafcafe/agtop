...
**Layout and focus**
- The list (**Agents**) keeps ≥25% width (≥30 columns). There's no split when the pane would be under 84 columns. The user can resize (shift+←/→ or alt+←/→ with nothing typed, drag the divider, `/width 30%`). `#view split|agent|list` picks the layout outright. Stepping or dragging past either end leaves one side alone: past 75% is Agents alone, past 25% is the Session alone; the opposite step brings the split back. Whichever layout you land on is kept across restarts (`Config.View`, set with `Config.SetView`, which also sets `ListOnly` and `ChatFull`): the Session alone opens agtop on the picked agent's Session and opens later Sessions alone too; Agents alone opens Sessions as you last had one (`chatAlone()`). Closing (esc, ←, tab) goes back to Agents alone; from the Session alone that's a peek (`m.peekFrom`, `peeking()`): ↑↓ pick, enter or tab opens the picked agent alone again, esc goes back to the one you left. With `View` unset, agtop asks on launch (`viewSheet`); a `ListOnly` from before `View` existed counts as "list". While one side is alone on a screen wide enough to split, the hint row shows `shift+← · #view split` (or `shift+→`). Settings › General › Layout changes it too.
- Names for the parts:
  - **Agents** (left list)
