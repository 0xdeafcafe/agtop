...
`agtop open <id> --solo` is the view of that one session alone, under agtop's header, with the Session at the terminal's whole width and no Agents list. `ctrl+\` opens Efficiency, Machine and Settings as usual, and Agents is the session again. The keys that lead to other agents or open the list (`ctrl+z ctrl+n tab`) do nothing, and `, . < >` are typed into the box. `ctrl+k` searches every agent and place; going to another agent from it shows the list beside it, as `ctrl+6` does. `ctrl+6` (sent as `ctrl+^` by most terminals) shows Agents beside the session, to pick and answer another agent; `ctrl+6` again, or `esc` from the list, hides it and the view is back on the solo session. Outside solo, `ctrl+6` hides or shows Agents beside an open Session for the moment, while `#view` keeps the layout you chose. The message box has the keys from the start. `esc` never closes the view: on an empty box it takes the keys off the box (typing, `enter` or `→` brings them back), and `esc` again stops the running turn, as `ctrl+x` does. `ctrl+q` alone closes the view; the session keeps running. What is typed in the box, and a stashed message, is kept when the view closes and is back when it opens again. A stopped session shows its conversation and resumes with the first message.
